...
export const playModpack = createAsyncThunk('modpacks/playModpack', async (payload: { modpack }, { getState }) => {
...
export const extractZip = createAsyncThunk('modpacks/extractZip', async (payload: { zipPath: string, filename: string }) => {
  const response = await (window as any).electronAPI.extractZip(payload.zipPath, payload.filename);
  return response;
})

export const updateInstalledModpacks = createAsyncThunk('modpacks/updateInstalledModpacks', async () => {
...
export const modpacksSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder.addCase(fetchModpacks.fulfilled, (state, action) => {
      state.modpacks = action.payload.modpacks;
      console.log('action.payload', action.payload);
      state.java = action.payload.java;
    })
    builder.addCase(updateInstalledModpacks.fulfilled, (state, action) => {
