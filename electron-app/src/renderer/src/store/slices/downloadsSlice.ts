...
export const downloadFile = createAsyncThunk('downloads/downloadFile', async (payload: { url: string, filename?: string, isModpack?: boolean }, { rejectWithValue }) => {
  try {
    const response = await (window as any).electronAPI.downloadFile(payload.url, payload.filename || '');
...
const downloadsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder.addCase(fetchDownloadsPath.fulfilled, (state, action) => {
...
export const { setDownloadUrl, setDownloadFilename, updateDownloadProgress, resetCurrentDownload, clearError, setDownloadType } = downloadsSlice.actions;
